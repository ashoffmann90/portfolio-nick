...
function BehindTheScenes() {
  ...
  const onClick = e => {
    ...
    const picsChild = { width: `10em`, height: `25em`}
    const picsParent  = { marginLeft:'15px', width: `50em`, height: `10em`}
    const vidsChild = { width: `10em`, height: `25em`}
    const vidsParent  = { marginLeft:'15px', width: `50em`, height: `10em`}
    ...
    return (
        <div className='bts-container'>
            ...
            :
             <div className='bts-content-container'>
                ...
                <div className='bts-pics'>
                    {thumbnailArray.map((pic, index) => 
                          <img key={index} src={pic} alt='x' onClick={onClick}/>
                        )}
                </div>
            </div>
            }
                  ...
                  <HorizontalScroll>
                    {urlArray.map((vid, index) => 
                        <div style={vidsChild} key={index}>
                          <ReactPlayer className='bts-vid' url={vid} controls={true} width='75%' />
                        </div>
                        )}
                  </HorizontalScroll>
                </div>
            ...
            <div className='bts-content-container'>
                ...
                {urlArray.map((vid, index) => 
                          <ReactPlayer className='bts-vid' key={index} url={vid} controls={true} width='75%' />
                        )}
                </div>
            </div>
            }
